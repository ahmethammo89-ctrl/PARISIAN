...
**Customer app** — sign in with Google, email/password, or phone/OTP (all three on one login page, each fully independent — see Auth below), multiple saved addresses, a 5-category luxury service menu (Clothes, Upholstery, Shoes, Bags, Haute Couture) with an optional per-service catalog photo, per-item customization (fold/hanger, starch level, stain notes), a native-feel per-item photo upload (one slot per physical item, camera capture), live dynamic pricing (Normal/Express/Urgent), manual Whish Money (transfer + screenshot + transaction ID, staff-verified) or Cash on Delivery, and a WhatsApp fallback. The home dashboard is the hub: place a new order, browse a read-only **Menu & Prices** page, see every past/current order, and a **Messages** box (a WhatsApp-style thread, text or voice notes) for anything that isn't tied to one specific order — a complaint about a staff member, a general question — which reaches the admin team instantly. A per-order detail page lets a customer track that order and adjust an item's fold/starch/notes right up until staff confirms it — locked automatically from that point on.

**Admin dashboard** — an order queue with status/branch filters and search, a realtime toast (with a chime) the moment a customer checks out or sends a message, a full order detail view (customer, address, items, payments — including reviewing/confirming manual Whish proof screenshots, status history), one-click order status advancement, driver assignment (pickup + dropoff, any branch), per-item status control, printable QR/barcode tag sheets, a **Messages** inbox (one thread per customer, same text/voice chat as the customer side) for handling complaints and questions, a **Catalog** page (any staff) to add/edit categories and services with per-service trilingual names, prices, and a photo, and a **Users** page (admin/super_admin only) to create staff/driver/admin accounts directly, change roles/branch/active status, and reset anyone's password — no SQL needed for day-to-day team or catalog management. `super_admin` additionally gets a **Customers** tab on the Users page to see and manage every customer account (incl. Google/email sign-ups), including password resets for "I forgot my password" requests — plain `admin` and `staff` never see the customer list.
...
4. `supabase/05_service_images_chat_and_edit_lock.sql` — adds `services.image_url` plus the public `service-images` storage bucket (Admin → Catalog photo upload, shown in the customer service picker and the Menu & Prices page), the realtime new-order toast on the admin dashboard nav, and **narrows** the customer item-edit window back to `pending_confirmation` only (once staff confirms the order, the customer can no longer edit item options — this supersedes the wider window `04` introduced). Also creates an `order_messages` table for a per-order chat — since superseded by the general Messages inbox below and no longer used by the UI, but left in place (harmless) rather than dropped. **Required** for catalog photos, the edit-window fix, and the new-order alert — run it once against your existing project.
5. `supabase/06_support_chat_and_notifications.sql` — adds the `support_messages` table (one general text/voice thread per customer, RLS + realtime) that powers **Messages** on the customer dashboard and Admin → Messages, plus the private `chat-audio` storage bucket for voice notes (folder-scoped per customer, same pattern as `item-photos`). **Required** for Messages/complaints and voice notes to work — run it once against your existing project.
...
- **Admin alert sound**: the new-order/new-message chime (`components/NewOrderAlert.tsx`) is generated on the fly with the Web Audio API — nothing to host — but browsers block audio until the page has had at least one click/keypress, so it may stay silent for the very first alert after a fresh page load; any click on the admin UI (e.g. navigating a tab) unlocks it for the rest of the session. The realtime subscription itself also auto-resubscribes a few seconds after a dropped connection, and every alert triggers a background `router.refresh()` so the current page's data (order counts, order list, message previews) catches up without a manual reload.
