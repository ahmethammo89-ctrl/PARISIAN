...
**Customer app** — sign in with Google, email/password, or phone/OTP (all three on one login page, each fully independent — see Auth below), multiple saved addresses, a 5-category luxury service menu (Clothes, Upholstery, Shoes, Bags, Haute Couture) with an optional per-service catalog photo, per-item customization (fold/hanger, starch level, stain notes), a native-feel per-item photo upload (one slot per physical item, camera capture), live dynamic pricing (Normal/Express/Urgent), manual Whish Money (transfer + screenshot + transaction ID, staff-verified) or Cash on Delivery, and a WhatsApp fallback. A dashboard order list plus a per-order detail page let a customer track every order (including past/completed ones), message the branch directly through a realtime per-order chat, and adjust an item's fold/starch/notes right up until staff confirms the order — locked automatically from that point on.

**Admin dashboard** — an order queue with status/branch filters and search, a realtime "new order" toast the moment a customer checks out, a full order detail view (customer, address, items, payments — including reviewing/confirming manual Whish proof screenshots, a realtime per-order chat with the customer, status history), one-click order status advancement, driver assignment (pickup + dropoff, any branch), per-item status control, printable QR/barcode tag sheets, a **Catalog** page (any staff) to add/edit categories and services with per-service trilingual names, prices, and a photo, and a **Users** page (admin/super_admin only) to create staff/driver/admin accounts directly, change roles/branch/active status, and reset anyone's password — no SQL needed for day-to-day team or catalog management. `super_admin` additionally gets a **Customers** tab on the Users page to see and manage every customer account (incl. Google/email sign-ups), including password resets for "I forgot my password" requests — plain `admin` and `staff` never see the customer list.
...
4. `supabase/05_service_images_chat_and_edit_lock.sql` — adds `services.image_url` plus the public `service-images` storage bucket (Admin → Catalog photo upload, shown in the customer service picker), the `order_messages` table + RLS + realtime publication for the per-order chat (customer ↔ staff, shown on both the customer order page and Admin → order detail) and the realtime new-order toast on the admin dashboard nav, and **narrows** the customer item-edit window back to `pending_confirmation` only (once staff confirms the order, the customer can no longer edit item options — this supersedes the wider window `04` introduced). **Required** for the chat, catalog photos, and new-order alert to work — run it once against your existing project.
